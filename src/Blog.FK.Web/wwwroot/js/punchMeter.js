define([], function () {
    //if you can try yourself in https://github.com/intel/generic-sensor-demos
    ...
    var quotes = ["Soco não detectado", "Belo soco!", "Continue praticando os golpes!"];
    ...
    function onresult() {
        ...
        setGameText(getQuote(speedCalculator.result) + " Mexa seu celular para tentar novamente!");
    ...
    function main() {
        ...
        function startApp() {
            ...
            acl.addEventListener('error', error => {
                setGameText("Não foi possível obter os dados do sensor.");
            });
            acl.start();
        }
        ...
        if ('LinearAccelerationSensor' in window) {
            navigator.permissions.query({ name: "accelerometer" }).then(result => {
                if (result.state != 'granted') {
                    setGameText("Desculpe, não temos permissão para acessar os sensores no seu celular");
                    return;
                }
                startApp();
            }).catch(err => {
            ...
            setGameText("Seu browser não suporta sensores.");
            setMeasurement(0);
        }
    }
